...
public class SecurityController implements ISecurityController {
    ...
    public String createToken(UserDTO user) {
        try {
            ...
            // Tjek om værdierne ikke er null eller tomme
            if (ISSUER == null || TOKEN_EXPIRE_TIME == null || SECRET_KEY == null) {
                throw new ApiException(500, "Missing token configuration values");
            }

            // Generer token
            String token = tokenSecurity.createToken(user, ISSUER, TOKEN_EXPIRE_TIME, SECRET_KEY);

            // Tjek om token er null
            if (token == null) {
                throw new ApiException(500, "Token generation returned null");
            }
